...
function site_options_menu() {
    ...
    $developermode = get_option('developermode');   
    
    $get_developermode = filter_input(INPUT_GET, 'developermode');
    ...
    foreach($ters as $site_option) 
    {
        ...
        elseif($site_option['menu_name']!=$ters[0]['menu_name'] AND $site_option['is_procted']=='yes' AND ($developermode=='enable' OR $get_developermode=='enable'))
        {
            add_submenu_page( $ters[0]['page_slug'], $site_option['menu_name'], $site_option['menu_name'], 'administrator', $site_option['page_slug'], $site_option['page_slug'] );
        }        
    }
...
";
    ...
    elseif($site_option['is_procted']=='yes' AND ($developermode=='enable' OR $developermode=='enable'))
    {
        include $site_option['php_page_name'];
    }
